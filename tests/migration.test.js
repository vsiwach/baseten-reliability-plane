...
test('shadow → certify → promote, with rollback held for the route’s life', () => {
  const m = M.createMigration({ route: 'voice-agent', source: 'competitor-cloud',
  ...
  assert.strictEqual(m.serving, 'competitor-cloud', 'one click restores the original pool');
});
...
test('the gate refuses honestly: SLO miss → HOLD, no promotion possible', () => {
  ...
  assert.strictEqual(m.serving, 'competitor-cloud', 'route stays on the incumbent');
...
test('the same machine runs in either direction (no lock-in)', () => {
  const out = M.createMigration({ route: 'chat-prod', source: 'baseten-dedicated',
                                  target: 'competitor-cloud', slo, requiredSamples: 3 });
  for (let i = 0; i < 3; i++) out.feed(goodPair());
  out.certify();
  assert.ok(out.promote());
  assert.strictEqual(out.serving, 'competitor-cloud', 'migrate-out is one click too');
});

test('win-back fires only for external routes that would hold SLO cheaper on an operated pool', () => {
  const routes = [{ id: 'voice-agent', pool: 'competitor-cloud' },
                  { id: 'chat-prod', pool: 'baseten-dedicated' }];
  const pools = [
    { id: 'competitor-cloud', control: 'monitor-only', dedicated: true, usd_per_mtok: 10.19, ttft_p99_ms: 300 },
